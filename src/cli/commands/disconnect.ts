...
export function registerDisconnectCommand(
  ...
  program
    ...
    .action(async (host: string, options: DisconnectOptions) => {
      ...
      const plan = await prepareAntigravityDisconnect(dependencies, options.surface);
      writeLine(output, "AgentFold Antigravity disconnect");
      writeLine(output);
      ...
      const result = await applyAntigravityDisconnect(plan, dependencies);
